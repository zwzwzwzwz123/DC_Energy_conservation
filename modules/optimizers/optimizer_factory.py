...
class OptimizerFactory:
    ...
    def create_optimizer(
        ...
        Args:
            algorithm: 优化算法名称，支持：
                ...
                - 'simulated_annealing' 或 'sa': 模拟退火优化（无外部依赖）
    ...
    def get_all_algorithms_info() -> Dict[str, Dict[str, str]]:
        ...
        return {
            'bayesian': {
                'name': '贝叶斯优化',
                'description': '使用 Optuna 与 TPE 采样器，适合小规模参数空间的高效优化',
            ...
            'simulated_annealing': {
                'name': '模拟退火',
                'description': '通过逐步降温的随机搜索找到近似全局最优解',
                'dependencies': '无',
                'available': True
            },
            'sa': {
                'name': '模拟退火（简写）',
                'description': '与 simulated_annealing 相同',
                'dependencies': '无',
                'available': True
            },
            'genetic': {
